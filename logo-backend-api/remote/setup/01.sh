...
sudo -i -u postgres psql -c "CREATE DATABASE pglogo"
sudo -i -u postgres psql -d pglogo -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d pglogo -c "CREATE ROLE andrew WITH LOGIN PASSWORD '${DB_PASSWORD}'"
# Add a DSN for connecting to the greenlight database to the system-wide environment
# variables in the /etc/environment file.
echo "LOGOCOMP='postgres://andrew:${DB_PASSWORD}@localhost/pglogo'" >> /etc/environment
